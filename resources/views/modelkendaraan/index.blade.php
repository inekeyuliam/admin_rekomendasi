@extends('layouts.app', ['activePage' => 'modelkendaraan', 'titlePage' => __('')])
@section("content")
<div class="content">
            ...
            <div class="col-md-10">
                <div class="card">
                    <div class="card-header card-header-primary">
                        <h3 class="card-title ">Daftar Model Kendaraan</h3>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-10">
                            ...
                            <div class="col-md-2">
                            <a class="btn btn-primary btn-md pull-right" href="{{url('modelkendaraan/create')}}">Tambah Model Kendaraan</a> 
                            </div>
                            </div>
                            ...
                            <div class="table-responsive">
                                ...
                                <thead >
                                    ...
                                    <tbody>
                                    @foreach ($listmodel as $key=>$item)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$item->merk_kendaraan->nama_merk}}</td>
                                        <td>{{$item->nama_model}}</td>
                                        <td>{{$item->jenis_kendaraan}}</td>
                                        <td>{{$item->kapasitas}}</td>

                                        <td>
                                        <a href="{{url('modelkendaraan/'.$item->id.'/edit')}}"><i class="fa fa-edit"></i></a>
                                        </td>                         
                                        <td>
                                        <form method="POST" action="{{ url('modelkendaraan/'.$item->id) }}" id="form-hapus-{{ $item->id }}">
                                            {{ method_field("DELETE") }}
