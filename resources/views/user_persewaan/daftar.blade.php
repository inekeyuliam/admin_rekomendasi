        ...
        <section class="section section-variant-1 bg-default novi-background bg-cover"> 
            ...
            @foreach($persewaan as $item)
                ...
                <article class="event-default-wrap">
                    ...
                    <h5><a class="event-default-title" href="{{url('detailpersewaan/'.$item->id)}}">{{$item->nama_persewaan}}</a></h5><span class="heading-5"></span>
                    </div>
                </article>
                </div>
            @endforeach
          ...
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Filter Persewaan Kendaraan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>                
              <form method="POST" action="/filter/persewaan">
              <input type="hidden" name="_token" value="{{ csrf_token() }}">
              <div class="modal-body">
                <div class="form-check">
                <label> Jenis Kendaraan </label> </div>
                <div class="form-check ">
                <label><input type="checkbox" id="checkall" name="checkall">
                Semua</label><br>
                </div>
                <div class="form-check ">
                <label><input type="checkbox" id="tipe_ken" name="tipe_ken[]" value="Motor">
                Motor</label><br>
                </div>
                <div class="form-check ">
                <label><input type="checkbox" id="tipe_ken" name="tipe_ken[]" value="Mobil">
                Mobil</label><br>
                </div>
                <div class="form-check ">
                <label><input type="checkbox" id="tipe_ken" name="tipe_ken[]" value="Minibus/Bus">
                Minibus/Bus</label>
                </div><br>


                <div class="form-check">
                <label> Kapasitas Penumpang </label> </div>
                <div class="form-check">
                <label><input type="radio" id="kapasitas" name="kapasitas"> Semua</label>
                </div>
                <div class="form-check">
                <label><input type="radio" id="kapasitas" name="kapasitas" value="1"> 2 - 4</label>
                </div>
                <div class="form-check">
                <label><input type="radio" id="kapasitas" name="kapasitas" value="2"> 5 - 6</label>
                </div>
                <div class="form-check">
                <label><input type="radio" id="kapasitas" name="kapasitas" value="3"> > 6</label>
                </div><br>
                ...
                <div class="form-check">
                <label> <a data-toggle="modal" id="modalterdekat" data-target="#exampleModal3" href="#exampleModal3">Filter Berdasarkan Persewaan Kendaraan Terdekat</a> </label> </div>
        ...
        <div class="modal fade" id="exampleModal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
              ...
              <form method="POST" action="/filter/lokasi/persewaan">
              <input type="hidden" name="_token" value="{{ csrf_token() }}">
              <div class="modal-body">    
...
function check(checked = true) {
    const cbs = document.querySelectorAll('input[name="tipe_ken[]"]');
    cbs.forEach((cb) => {
        cb.checked = checked;
    });
}
